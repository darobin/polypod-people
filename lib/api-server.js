  ...
  constructor (config) {
    ...
    this.repo = new Repo({
      ...
      sharePolicy: async (peerID, docID) => {
        console.warn(`SHARE:`, peerID, docID);
        return false;
      },
    });
  }
  async run () {
    await Promise.all([this.dir, this.accountsDir, this.automergeDir].map(mkdir));
    const app = express();
    app.get('/', (req, res) => {
    ...
    app.post('/api/register', express.json(), async (req, res) => {
      ...
      if (!/^\w[\w.\-_]*$/.test(username)) return fail(res, 'Bad username');
      ...
      const userPath = join(this.accountsDir, `${username}.json`);
      if (await exists(userPath)) return fail(res, 'User exists', 401);
      const p = await createPerson(this.repo, { name });
